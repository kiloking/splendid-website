...
const Contact = () => {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    ...
    const senddata = {
      to: "dz@da-city.com.tw",
      subject: "大漁建設官網表單:" + data.subject,
      text: text,
    };
    try {
      const response = await fetch(
        "https://sendmail-api-umber.vercel.app/send-email",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="md:mt-[88px] px-10 md:px-20">
      <div className="w-full aspect-[16/5] overflow-hidden  bg-zinc-400 rounded-lg relative">
        <Image
          src="https://pub-a73f7d8384414e33a55eebf09fd805cd.r2.dev/contactBanner.jpg"
          alt="image"
          width={200}
          height={24}
          priority={false}
        ...
        <div className=" flex flex-col items-start gap-4  absolute md:top-10 md:left-10 top-2 left-2 z-10">
          <div
            className=" bg-[#DC0000] text-white text-sm px-3 py-1 rounded-md"
            data-aos="fade-left"
          >
            聯絡大漁
          </div>
          <div
            className="text-3xl md:text-5xl font-thin  tracking-tight text-white "
            data-aos="fade-left"
          >
            CONTACT US
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="text-[#5C2C1F] font-bold text-sm w-full mx-auto my-4">
              <div className=" flex mt-2 text-xs md:text-base items-center justify-center">
                ...
                />
                <div
                  className="text-black ml-2   underline-offset-2 cursor-pointer"
                  onClick={() => setIsOpen(true)}
                >
                  {" "}
                  <span className="underline">個人資料聲明事項</span>
                  ，我同意通過電話或郵件方式與我聯絡
                </div>
              </div>
            </div>
            ...
            <button
              type="submit"
              className=" bg-white rounded-full w-full py-2 hover:bg-zinc-200 relative"
            >
              送出{" "}
              {isSending && (
        ...
        <Modal>
          ...
          <div className=" absolute z-10 top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%] w-3/5 h-4/5 bg-white mx-auto px-10 py-10 rounded-sm border-slate-200 overflow-y-scroll">
            ...
            <div className="text-base  text-zinc-500 leading-8 mb-6 mt-6">
              本公司依據個人資料保護法，向您告知以下隱私權政策規範，若您勾選「我同意隱私權政策」，將表示您已同意本公司之隱私權政策，並同意本公司依以下告知事項，就您的個人資料進行蒐集、處理及利用。您可以自由選擇是否提供相關個人資料，若您拒絕，本公司將可能無法執行本告知事項蒐集目的之業務，致無法向您提供相關服務：
            </div>
